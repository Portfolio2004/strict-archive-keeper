  ...
  const learningCards = [
    {
      ...
      description:
        "Developing a Relational-Database that includes core operations",
      color: "text-primary",
    },
    {
      ...
      icon: <Network className="h-6 w-6" />,
      description:
        "Firewall configuration, Routing, Networking, Port-Forwarding, VPN setup",
      color: "text-accent",
    },
    {
      ...
      icon: <Cloud className="h-6 w-6" />,
      description:
        "Multi-cloud strategies, cost optimization, and scalability design",
      color: "text-primary",
    },
  ];
        ...
        <ScrollReveal direction="up" delay={0.2}>
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold mb-6 tracking-tight">
              ...
              <span className="font-sans">Excellence</span>
            </h2>
          </div>
        </ScrollReveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {skillCategories.map((category, index) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={
                  category.centerCard ? "md:col-start-2 lg:col-start-2" : ""
                }
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {learningCards.map((card, index) => (
                <motion.div
                  ...
                  whileTap={{ scale: 0.98 }}
                >
                  <Card className="min-w-[320px] bg-card/60 border-border/50 hover:border-primary/40 transition-all duration-500 golden-glow-hover backdrop-blur-lg relative overflow-hidden group">
                    <motion.div
                      className="absolute inset-0 bg-gradient-to-br from-primary/5 to-accent/5 opacity-0 group-hover:opacity-100"
                      transition={{ duration: 0.5 }}
                    />
                    <CardHeader className="pb-4 relative z-10">
                      <CardTitle className="flex items-center gap-3 text-lg">
                        <motion.div
                          className={`p-2 rounded-lg bg-card border border-border/50 ${card.color} relative overflow-hidden`}
                          whileHover={{
                            rotateY: 180,
                            backgroundColor: "hsl(var(--primary) / 0.1)",
                          }}
